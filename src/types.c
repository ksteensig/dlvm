...
#define ARITHMETIC(r1, r2, op) {								\
	if (!is_number(r1) || !is_number(r2)) {						\
		return NULL;											\
	} else if (is_float(r1) && is_float(r2)) {					\
		return op##_floats((tfloat_t *)r1, (tfloat_t *)r2);		\
	} else if (is_int(r1) && is_int(r2)) {						\
		return op##_ints((tint_t *)r1, (tint_t *)r2);			\
	} else if (is_int(r1) && is_float(r2)) {					\
		return op##_floats(int_to_float(r1), (tfloat_t *)r2);	\
	} else if (is_float(r1) && is_int(r2)) {					\
		return op##_floats((tfloat_t *)r1, int_to_float(r2));	\
	}															\
	return NULL;												\
}

#define COMPARISON(r1, r2, op)										\
	if (is_int(r1) && is_int(r2)) {									\
		return int_##op((tint_t *)r1, (tint_t *)r2);				\
	} else if (is_float(r1) && is_float(r2)) {						\
		return float_##op((tfloat_t *)r1, (tfloat_t *)r2);			\
	} else if (is_int(r1) && is_float(r2)) {						\
		return float_##op(int_to_float(r1), (tfloat_t *)r2);		\
	} else if (is_float(r1) && is_int(r2)) {						\
		return float_##op((tfloat_t *)r1, int_to_float(r2));		\
	}																\
	return NULL;
...
tfloat_t *int_to_float(ttype_t *r1) {
    return (tfloat_t *)init_float(((tint_t *)r1)->v);
}
...
ttype_t *less_than(ttype_t *r1, ttype_t *r2) {
	COMPARISON(r1, r2, less_than);
}

ttype_t *greater_than(ttype_t *r1, ttype_t *r2) {
	COMPARISON(r1, r2, greater_than)
}

ttype_t *equals(ttype_t *r1, ttype_t *r2) {
	COMPARISON(r1, r2, equals);
}
